...
lvmdiskscan 말고 df -h로 계속 확인해줘라, 파일시스템이 같이 증가했는지 확인하기위해서
...
Daemon = 1.run level  2. super daemon   3. standalone   3가지 방식이 있다.
...
anacron은 실행하지 않을때 다시 실행하도록 함.check기능 [ cat /etc/anacrontab ] 
```
